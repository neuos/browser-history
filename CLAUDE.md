...
### Live deployment — oracle, 2026-09-23

`dotnet-backend` is deployed and internet-reachable at `https://browser-history.oracle.neuhuber.eu` (rootless Docker on the `oracle` host, behind its existing Caddy reverse proxy — see that host's own `SERVER_DOCUMENTATION.md` for the general server layout). Getting a from-scratch deployment actually working surfaced three more real bugs invisible to every fixture/dev-database test in this file so far, because a real fresh deployment has no pre-existing, hand-migrated database or built image to hide behind: `InitializeDatabaseAsync()` (EF migrations) was never called from `Program.cs` at all; the `Dockerfile` published with `PublishAot=true` despite `ApiJsonContext` being known-incomplete (same class of bug as the reflection fix above, but for the container image specifically); and its `ENTRYPOINT` ran the glibc-linked apphost directly on an Alpine (musl) runtime image. All three fixed — see git log on `dotnet-backend/` around 2026-09-23 for specifics. **Any future from-scratch deployment (a new host, a wiped container, CI) is the one path this repo's test suite doesn't cover — verify it for real, don't assume a green `dotnet test` run means a fresh deployment will actually boot.**

`extension/`'s Firefox build now has a stable identity and has been signed and distributed, live-verified installed on a real Firefox for Android device: `wxt.config.ts` sets `browser_specific_settings.gecko.id = 'browser-history-sync@neuhuber.eu'` (`strict_min_version: '109.0'`) so future AMO-signed builds keep the same extension ID instead of getting a random one each time. `bun run build:firefox` produces `.output/firefox-mv2/`; that directory was signed via `bunx web-ext sign --source-dir=.output/firefox-mv2 --channel=unlisted` against Mozilla's AMO API (self-hosted/unlisted signing — this is a personal build, not going through AMO review for public listing) to produce a real `.xpi`, since Firefox removed unsigned-extension sideloading in Firefox 79 and Firefox Android enforces the same signing requirement. The signed `.xpi` was uploaded to the user's self-hosted Gokapi instance (`files.neuhuber.eu`) via its chunked upload API — Gokapi's `/d?id=...` link is an HTML landing page (wrong for this), the raw file needs the `/downloadFile?id=...` hotlink so the browser sees `content-type: application/x-xpinstall`. AMO signing credentials (JWT issuer + secret) and the Gokapi API key are treated as durable secrets, not committed to this repo.

**Installing on Firefox for Android**: the AMO-recommended "custom collection" override (Settings → About Firefox → tap the version 5x to unlock a debug menu → a "Custom Add-on collection" field pointing at an AMO username + collection slug) turned out to be unnecessary and didn't work cleanly in practice. What actually worked: Firefox for Android has a built-in **"Install add-on from file"** option (under the extensions/add-ons settings screen) that installs a downloaded, AMO-signed `.xpi` directly from local storage — no collection, no curated-list restriction. This is the simplest path for any future self-distributed build on this target; try it first before the collection workaround.

AMO collections themselves can't be created through the JWT-based signing API the user's credentials use — that API key is scoped to add-on submission/signing endpoints only (`GET`s like `/api/v5/addons/addon/{guid}/` work, `POST /api/v5/accounts/account/{id}/collections/` 401s). Creating a collection needs the real logged-in AMO web session, not something to script around.
...
- **`BrowserHistory.Application`** — CQRS via MediatR. Organized by feature under `Features/{Auth,Devices,Sync}/`, each with `Commands/`, `Queries/`, and `Models/` (feature-scoped DTOs — this split replaced an earlier flat DTO layout because of naming collisions across features). `Common/Behaviors/ValidationBehavior.cs` runs FluentValidation validators as a MediatR pipeline behavior before every handler. (A `History`/`Pages` feature — server-side `HistoryNode`/`Page` entities, repositories, and `/api/v1/history`+`/api/v1/pages` REST endpoints — existed from the original design doc but was removed 2026-09-23: the extension's sync flow never called it, confirmed dead against live production data (303 real `SyncEvents`, 0 rows in either table), and it was fully redundant — see "Sync architecture" below for why.)
...
**Sync architecture, confirmed 2026-09-23**: the server is a pure event log (`SyncEvents` table) — it never materializes anything into its own queryable history store. Cross-device search happens entirely client-side: `SyncClient.applySyncEvent()` writes every received event (the device's own captured visits *and* events relayed from other devices) into the same `HistoryRepositoryIndexedDB`/`PageRepositoryIndexedDB` with no device-scoping, so `HistoryService.getHistoryEntries()` (what the popup's search box calls) already searches merged history across every synced device. This is why the server-side `History`/`Pages` REST feature (see above) was safe to delete — the real cross-device search this project's goal calls for was already working without it.
