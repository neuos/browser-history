...
`extension/`'s Firefox build (`bun run build:firefox`) has not been distributed to a real device yet — see whichever session did that for the current state of Firefox Android's signing requirement (Mozilla removed unsigned sideloading in Firefox 79; a build needs to go through AMO signing, even for self-distribution, before Firefox Android will install it from a downloaded file).
